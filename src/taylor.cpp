...
template <typename T>
std::tuple<taylor_outcome, T, T, std::size_t> taylor_adaptive_impl<T>::propagate_until_impl(T t, std::size_t max_steps,
                                                                                            T max_delta_t)
{
    using std::isfinite;
    using std::isnan;
    ...
    while (true) {
        ...
        const auto dt_limit = t_dir ? std::min(max_delta_t, t - m_time) : std::max(-max_delta_t, t - m_time);
        const auto [res, h] = step_impl(dt_limit, false);
...
taylor_adaptive_impl<T>::propagate_grid_impl(const std::vector<T> &grid, std::size_t max_steps, T max_delta_t,
                                             std::function<void(taylor_adaptive_impl &, std::size_t)> callback)
{
    using std::abs;
    using std::isfinite;
    using std::isnan;
    ...
    for (decltype(grid.size()) cur_grid_idx = 1; cur_grid_idx < grid.size();) {
        ...
        while (true) {
            ...
            if (cur_tt >= t0 && cur_tt <= t1) {
                ...
                if (callback) {
                    callback(*this, boost::numeric_cast<std::size_t>(cur_grid_idx));
                }
            } else {
        ...
        const auto dt_limit
            = t_dir ? std::min(max_delta_t, grid.back() - m_time) : std::max(-max_delta_t, grid.back() - m_time);
        const auto [res, h] = step_impl(dt_limit, true);
